                            ...
                            int col_stride, int i, int j, int k, int l)
{
    ...
    for (ii = 0; ii < k; ii++)
    {
        for (jj = 0; jj < l; jj++)
        {
            if (CSR_double_insert_entry(A, val[ii * row_stride + jj * col_stride], i + ii, j + jj))
                return -1;
        }
    }
                   ...
                   double *C, int C_row_stride, int C_col_stride)
{
    ...
    for (i = 0; i < K; i++)
    {
        CSR_double_mv(alpha, A, &B[i * B_row_stride], B_col_stride, &C[i * C_row_stride], C_col_stride);
    }
}

/*
 * Transpose multiply: A is NxM, B is NxK, C is MxK.
 *
*/
