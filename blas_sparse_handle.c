...
int BLAS_usgp(blas_sparse_matrix A, int pname)
{
    ...
    {
    case (blas_complex):
        return SPBLASI_is_complex(S->prop_);

    case (blas_real):
        return SPBLASI_is_real(S->prop_);

    case (blas_double_precision):
        return SPBLASI_is_double_precision(S->prop_);

    case (blas_symmetric):
        return SPBLASI_is_symmetric(S->prop_);

    case (blas_lower_triangular):
        return SPBLASI_is_lower_triangular(S->prop_);

    case (blas_upper_triangular):
        return SPBLASI_is_upper_triangular(S->prop_);

    case (blas_valid_handle):
        return SPBLASI_Matrix_state(S) == valid ? 1 : 0;
...
int BLAS_ussp(blas_sparse_matrix h, int pname)
{
    ...
    {
    case (blas_complex):
        SPBLASI_set_complex(&(S->prop_));
        break;

    case (blas_real):
        SPBLASI_set_real(&(S->prop_));
        break;
    ...
    case (blas_lower_symmetric):
        SPBLASI_set_lower_symmetric(&(S->prop_));
        break;

    case (blas_upper_symmetric):
        SPBLASI_set_upper_symmetric(&(S->prop_));
        break;
    ...
    case (blas_upper_triangular):
        SPBLASI_set_upper_triangular(&(S->prop_));
        break;

    default:
        BLAS_ASSERT_RETURN(0, -1);
    }
